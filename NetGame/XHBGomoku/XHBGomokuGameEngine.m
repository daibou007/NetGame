...
-(BOOL)playerChessDown:(NSInteger)row line:(NSInteger)line
{
    BOOL isOk = NO;
...
-(void)computerPrepareChess:(NGGameStep*)step{
//    if (self.delegate&&[self.delegate respondsToSelector:@selector(game:statuChange:)]) {
//        [self.delegate game:self statuChange:XHBGameStatuComputerChessing];
//    }
    self.gameStatu=XHBGameStatuComputerChessing;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        XHBGomokuChessPoint*point= [self.chessBoard pointAtRow:[step.tapRow intValue]  line:[step.tapLine intValue]];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.playerFirst) {
                //赋值
                XHBGomokuChessElement * element=[XHBGomokuChessElement getChess:XHBGomokuChessTypeWhite];
                point.chess=element;
                isOver = [self chessDown:point chessType:XHBGomokuChessTypeWhite];
            }else{
                //赋值
                XHBGomokuChessElement * element=[XHBGomokuChessElement getChess:XHBGomokuChessTypeBlack];
                point.chess=element;
                isOver = [self chessDown:point chessType:XHBGomokuChessTypeBlack];
            }
            if (isOver) {
               //TODO OVER
